...
/* ── Booking ───────────────────────────────────────────────────────────────
 * Spotlight Links onboards every business through a conversation now, not a
 * checkout. This Google Calendar appointment schedule is the single entry point
 * for inbound interest, and the blog is the main thing pointing at it — so the
 * URL lives here once and reaches every surface that needs it, including the
 * prerendered blog HTML and /llms.txt.
 *
 * `scripts/marketing-data.mjs` reads this block as text (regex, no TS loader),
 * so keep each field on one line as `key: 'value',`.
 * ────────────────────────────────────────────────────────────────────────── */
