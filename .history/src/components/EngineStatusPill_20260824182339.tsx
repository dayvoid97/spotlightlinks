...
export function EngineStatusPill() {
  ...
  return (
    <div className="border-line bg-surface-2/60 hidden items-center gap-3 rounded-full border px-3 py-1.5 md:flex">
      {engines.map((e) => (
        <div
          key={e.key}
          className="flex items-center gap-1.5"
          title={data.keys[e.key] ? `${e.label} key configured` : `${e.label} key missing`}
        >
          <span
            className={clsx(
          ...
          />
          <span className="text-ink-50 text-[11px]">{e.label}</span>
        </div>
      ))}
    </div>
  )
