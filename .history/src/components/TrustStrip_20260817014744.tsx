...
export function TrustStrip() {
  ...
  return (
    <section className="border-line bg-surface/90 relative w-full border-y backdrop-blur-md overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6">
        <div className="divide-line grid grid-cols-1 divide-y md:grid-cols-3 md:divide-y-0 md:divide-x">
          {trustSignals.map((item) => {
            const Icon = item.icon
            return (
              <div
                key={item.title}
                className="group hover:bg-surface-2/50 flex items-center justify-center gap-4 py-2.5 px-4 transition-all duration-300"
              >
