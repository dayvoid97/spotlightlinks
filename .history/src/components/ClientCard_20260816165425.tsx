...
const engineLabel: Record<string, string> = { gemini: 'Gemini', anthropic: 'Claude', perplexity: 'Perplexity' }

export function ClientCard({ client }: { client: ClientSummary }) {
  const mentionRate = client.totalRuns > 0 ? Math.round((client.validRuns / client.totalRuns) * 100) : null

  return (
    <Link to={`/clients/${client.slug}`}>
      <Card className="group h-full transition hover:border-brand hover:bg-surface-2">
        <CardBody className="flex h-full flex-col gap-3 pt-5">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2.5 min-w-0">
              {client.logoUrl ? (
                <img src={client.logoUrl} alt="" className="size-9 shrink-0 rounded-lg object-cover" />
              ) : (
                <div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-surface-2 text-xs font-semibold text-ink-50">
          ...
          <div className="mt-auto grid grid-cols-3 gap-2 border-t border-line pt-3 text-center">
            <div>
              <p className="text-sm font-semibold text-ink">{client.promptCount}</p>
              <p className="text-[10px] text-ink-50">prompts</p>
            </div>
            <div>
              <p className="text-sm font-semibold text-ink">{mentionRate !== null ? `${mentionRate}%` : '—'}</p>
              <p className="text-[10px] text-ink-50">valid runs</p>
            </div>
            <div>
              <p className="text-sm font-semibold text-ink">{client.probesToday}/2</p>
              <p className="text-[10px] text-ink-50">today</p>
            </div>
          </div>

          <p className="text-[11px] text-ink-30">
            {client.lastProbedAt ? (
              <>Last probed {formatDistanceToNow(new Date(client.lastProbedAt), { addSuffix: true })}</>
            ) : (
