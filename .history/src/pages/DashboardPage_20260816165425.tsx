 ...
 * Returns every client the signed-in user can see: their own private
...
export default function DashboardPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (c) => c.name.toLowerCase().includes(q) || c.slug.includes(q) || c.city?.toLowerCase().includes(q)
    )
  }, [data, query])
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-ink">Managed assets</h1>
          <p className="text-sm text-ink-50">Businesses you're probing for AI search visibility.</p>
        </div>
        <Link to="/clients/new">
          <Button>
