        ...
        {isMachine && articleMachineMeta ? (
          ...
          <>
            <header className="mt-4">
              ...
              {/* Scaled down heading size from text-4xl/sm:text-5xl to text-2xl/sm:text-3xl */}
              <h1 className="text-ink text-2xl font-semibold sm:text-3xl tracking-tight leading-tight">
                {post.title}
              </h1>

              {/* Reduced lead text size from default lead size to text-base/sm:text-lg */}
              {post.subtitle && (
                <p className="text-ink-50 mt-3 text-base sm:text-lg leading-relaxed">
                  {post.subtitle}
                </p>
              )}

              {/* Made meta text slightly smaller (text-xs) */}
              <div className="text-ink-30 mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
            ...
            {heroImage && (
              <img
                ...
                className="border-line mt-6 w-full rounded-lg border"
              />
            )}

            {/* Scaled body font sizes down using Tailwind prose utility classes or inline sizing */}
            <div
              className="blog-body text-ink-70 mt-8 text-sm sm:text-base leading-relaxed [&_p]:mb-4 [&_blockquote]:text-sm [&_blockquote]:my-4"
              dangerouslySetInnerHTML={{ __html: html }}
            />

            <BookDemoCta source={`blog:${slug}`} className="mt-10" />
          </>
        )}
      </article>
