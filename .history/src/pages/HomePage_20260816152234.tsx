...
/**
 * Public homepage. Unlike everything under <ProtectedRoute>, this route is
 * reachable whether or not anyone is signed in — see docs/11-homepage-and-blog.md.
 * The only thing that changes with auth state is the single CTA button, per
 * spec: signed out gets "Sign Up or Log In", signed in gets "Go to Dashboard".
 */
...
function CtaButton({
  ...
  if (loading) return <div className="h-9 w-32 animate-pulse rounded-lg bg-ink-700" />
  return (
...
function SiteHeader({ user, loading }: { user: ReturnType<typeof useAuth>['user']; loading: boolean }) {
  return (
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6">
        <div className="flex items-center gap-2">
          ...
          <div className="leading-none">
            ...
            <p className="text-[10px] text-gray-500">by Spotlight Links</p>
          </div>
        </div>
...
function Hero({ user, loading }: { user: ReturnType<typeof useAuth>['user']; loading: boolean }) {
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute -left-32 -top-32 size-96 rounded-full bg-violet-glow/20 blur-[120px]" />
      <div className="pointer-events-none absolute -bottom-32 right-0 size-96 rounded-full bg-cyan-glow/10 blur-[120px]" />
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 px-4 py-16 sm:px-6 sm:py-24 md:grid-cols-2 md:py-28">
        <div>
          ...
          <p className="mt-5 max-w-md text-base text-gray-400">
            Find out exactly what ChatGPT, Gemini, Claude, and Perplexity tell customers about
            your business — and fix what's costing you the recommendation.
          </p>
        ...
        <div className="relative flex items-center justify-center md:justify-end">
          <div className="pointer-events-none absolute right-0 size-72 rounded-full bg-violet-glow/25 blur-[100px] md:size-96" />
          <img
            src="/679.png"
            alt=""
...
function TrustStrip() {
  const items = [
    '3 AI engines audited per cycle',
    'Full cycle in 10 minutes',
    'Every claim carries a source URL',
  ]
  return (
    <div className="border-y border-ink-border bg-ink-900/40">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-8 gap-y-2 px-4 py-4 text-center text-xs text-gray-500 sm:px-6">
        {items.map((item) => (
    ...
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">AEO, GEO, and what actually gets a business recommended by AI.</p>
        </div>
      </div>
