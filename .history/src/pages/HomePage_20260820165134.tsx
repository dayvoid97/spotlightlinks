...
import { GlassmorphicIntakeDrawer } from '../components/marketing/GlassmorphicIntakeDrawer'
import { QueensBanner } from '../components/marketing/QueensBanner'
import { saveOnboardingDraft, emptyOnboardingForm } from '../lib/onboarding-draft'
...
const homepageMachineMetadata: PageMachineMetadata = {
  ...
  h1: 'Someone nearby is deciding where to go. Make sure they hear your name.',
  description:
...
export default function HomePage() {
  ...
  function handleOpenDrawer(biz: string, zip: string) {
    setIntakeBusinessName(biz)
    setIntakeZip(zip)
    setDrawerOpen(true)
  }
  ...
  return (
    <div className="bg-surface min-h-screen">
      ...
      {isMachine ? (
        ...
        <>
          <Hero loading={loading} onOpenDrawer={handleOpenDrawer} />
          <QueensBanner />
          <TrustStrip />

          <GlassmorphicIntakeDrawer
            isOpen={drawerOpen}
            onClose={() => setDrawerOpen(false)}
            initialBusinessName={intakeBusinessName}
            initialZip={intakeZip}
            user={user}
          />
...
function Hero({
  ...
  loading: boolean
  onOpenDrawer: (businessName: string, zip: string) => void
}) {
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-4xl flex-col items-center justify-center px-4 py-16 text-center sm:px-6 ">
        <div className="flex max-w-3xl flex-col items-center">
          <h1 className="text-ink text-2xl font-semibold leading-[1.1] sm:text-5xl md:text-6xl">
            Make sure a potential customer <span>finds your business</span>
            <br />
          </h1>

          <p className="text-ink-50 mt-6 max-w-2xl text-lg sm:text-xl">
            Every day, people around your block search Google — and now ask AI assistants — for
            exactly what you sell. Spotlight Links shows you whether your business comes up, how you
            stack up against the shop down the road, and what to fix. In plain English, for $79 a
            month.
          </p>

          {/* High-Converting Interactive Input Bar */}
          <div className="mt-8 w-full max-w-2xl">
            <HeroInteractiveInput loading={loading} onOpenDrawer={onOpenDrawer} />
          </div>
        </div>
      </div>
    </section>
...
function HeroInteractiveInput({
  ...
  /**
   * Hands off to the intake drawer rather than navigating straight to
   * /get-started. The drawer was already mounted on this page and wired to a
   * callback the hero never called, so it was unreachable dead UI; this is the
   * step where someone confirms the ZIP and lets AI draft their bio before
   * committing to the longer form.
   */
  function handleSubmit(e: React.FormEvent) {
    ...
    const storyText = [
      trimmedName ? `Business Name: ${trimmedName}` : '',
      trimmedZip ? `ZIP Code: ${trimmedZip}` : '',
    ...
    onOpenDrawer(trimmedName, trimmedZip)
  }
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between pt-1">
          <button
            type="submit"
            className="group/btn from-brand to-brand-dark shadow-brand/25 hover:shadow-brand/40 relative inline-flex w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r px-6 py-3.5 text-sm font-bold text-white shadow-lg transition-all duration-300 active:scale-[0.99] sm:w-auto"
          >
          ...
          <button
            ...
            className="text-ink-50 hover:text-ink hover:underline flex items-center justify-center gap-1.5 text-xs font-semibold py-2 px-3 transition-colors"
          >
      ...
      <div className="text-ink-50 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-xs font-medium">
        <span className="flex items-center gap-1">
          <ShieldCheck className="text-brand size-3.5" /> Takes about two minutes
        </span>
